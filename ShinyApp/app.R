...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    position = 'fixed-top',
    windowTitle = "Stable Fine-Mapping Visualization",
    ...
    tabPanel(
      title = "Home",
      tagList(lapply(1:6, function(i) br())),
      fluidRow(
        column(width = 3),
        column(width = 6,
          div(style = "text-align:center;
            ...
            h1("A Comparison of Stable and Residualized Fine-mapping Approaches", align = 'center'),
            br(),
            div(
              style = "text-align:center;",
              actionButton("select_gene", "Select Gene", icon("dna"),
      ...
      tagList(lapply(1:3, function(i) br())),
      h4(HTML("A.J. Aw, L.C. Jin, N.M. Ioannidis, Y.S. Song (2026) 'The impact of stability considerations on genetic fine-mapping.' <i>eLife</i> RP88039."), align = 'center'),
      h4("Created by: Alan Aw and Lionel Jin. Last updated: Feb 18, 2026", align = 'center'),
      tagList(lapply(1:4, function(i) br())),
      fluidRow(
        column(4),
        column(4, align = "center",
          tags$a(href = 'https://github.com/songlab-cal/StableFM',
    ...
    # Tab 1: Single Gene Comparison (with sidebar)
    tabPanel(
      title = "Single Gene Comparison",
      titlePanel("Single Gene Fine-Mapping Comparison"),
      sidebarLayout(
        sidebarPanel(
          ...
          actionButton("geneExpressionShowBtn", "Show Summary"),
          actionButton("geneExpressionHideBtn", "Hide Summary"),
        ...
        mainPanel(
          ...
          fluidRow(
            ...
            fluidRow(
              column(6,
                h3("3. Compare Functional Annotations"),
                fluidRow(
                  column(6, selectInput(inputId = "func_annot_class",
    ...
    # Tab 2: Genome-Wide Comparison
    tabPanel(
      title = "Genome-Wide Comparison",
      titlePanel("Genome-Wide Fine-Mapping Comparison"),
      p("Compare functional annotation scores across all GEUVADIS genes."),
      h3("Make selections below"),
      fluidRow(
        column(4, selectInput(inputId = "gw_func_annot_class",
      ...
      actionButton("funcDensityPlotShowBtn", "Show Plot"),
      actionButton("funcDensityPlotHideBtn", "Hide Plot"),
      hidden(plotOutput(outputId = "funcDensityPlot", height = "400px")),
      ...
      actionButton("funcAnnotPlotShowBtn", "Show Plot"),
      actionButton("funcAnnotPlotHideBtn", "Hide Plot"),
      hidden(plotOutput(outputId = "funcAnnotPlot", height = "400px"))
    ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$select_gene, {
    updateTabsetPanel(session, "tabset", selected = "Single Gene Comparison")
  })

  observeEvent(input$learn_more, {
    updateTabsetPanel(session, "tabset", selected = "Methodology")
  })
